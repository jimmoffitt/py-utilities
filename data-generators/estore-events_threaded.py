...
def send_event_worker():
    """Worker function to generate and send events."""
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        while True:
            # Schedule 10 event send tasks per second
            futures = [executor.submit(send_event_worker) for _ in range(10)]
            concurrent.futures.wait(futures)  # Wait for all threads to complete

            # Record totals to the database periodically (if enabled)
            if options.get("write_to_postgres", False) and time.time() - last_totals_update >= DB_UPDATE_INTERVAL_MINUTES * 60:
                record_totals_to_db()
                last_totals_update = time.time()

            # Sleep for 1 second before the next batch of events
            time.sleep(1)
